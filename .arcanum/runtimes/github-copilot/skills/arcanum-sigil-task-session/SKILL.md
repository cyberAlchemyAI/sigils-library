...
description: Run the installed Arcanum sigil task-session through Necronomicon.
...
<objective>
Run the installed Arcanum sigil task-session through Necronomicon while preserving registry, route, and observability contracts.
</objective>

<context>
Arcanum is installed at .arcanum/ in this repository. Necronomicon is the runtime authority at .arcanum/necronomicon/. The selected definition is .arcanum/necronomicon/arcana/task-session/SKILL.md.
</context>

<process>
1. Read .arcanum/necronomicon/REGISTRY.md and .arcanum/necronomicon/OBSERVABILITY.md.
2. Read .arcanum/necronomicon/arcana/task-session/SKILL.md.
3. For sigils, also read the sibling README.md when it exists. For spells, follow the spell file directly.
4. Execute only the installed sigil task-session unless the definition explicitly delegates or the user asks to route elsewhere.
