...
- Build one reusable library spell under `arcanum/spells/invoke/README.md`.
- Keep the active development pack under `arcanum/spells/invoke/development/` until Spellcraft validation and release approval promote canonical spell changes.
...
### 7. Implementation Layering Companion Contract

Every invoke run should track layering status:

- `layering_artifact_path`,
- `layering_status` (`seeded`, `updated`, `missing-gap-recorded`),
- `active_layer_window` (`L0`, `L1`, `L2`, `L3`, or `n/a`),
- `layer_decisions_snapshot`,
- `layering_unresolved_gaps`.

Default behavior:

- `define` and `design` may seed a new layering artifact or record a layering gap.
- `plan`, `full`, and `validate` require a layering artifact and explicit layer decision snapshot.
- Any missing required layering artifact blocks completion for `plan`, `full`, or `validate` until resolved or explicitly deferred by decision record.

### 8. Work-Pack Manifest Companion Contract

Every invoke planning run should track work-pack status:

- `work_pack_artifact_path`,
- `work_pack_output_mode` (`single-file` or `split`),
- `work_pack_mapping_status` (`mapped-from-plan`, `updated`, `missing-gap-recorded`),
- `work_pack_execution_ref` (`execution-pack.md` path or `n/a`),
- `work_pack_unresolved_gaps`.

Default behavior:

- `plan`, `full`, and `validate` require a work-pack artifact generated from implementation-plan mapping.
- low complexity may keep a single-file work-pack with `execution-pack` optional.
- medium/high complexity require split work-pack plus execution-pack reference and baseline wave coverage.
- missing required work-pack artifacts block completion for `plan`, `full`, or `validate` until resolved or explicitly deferred by decision record.
...
- generate or update implementation-layering artifact from the implementation-layering transmutation contract,
- generate or update work-pack artifact from implementation-plan mapping,
- generate or update execution-pack when work-pack output mode is split or complexity is medium/high,
...
- work-pack artifact path and implementation-plan mapping status,
- implementation-layering artifact path and layer decision status,
- decisions recorded,
...
- track unsupported new family requests as explicit candidate-template gaps instead of treating them as implemented,
...
- fail release validation when a required template family lacks validation evidence or approved candidate fallback evidence,
...
| Work-pack mapping from implementation-plan               | work-pack tasks, layers, blockers, and validation gates are traceably mapped from implementation-plan sections                       |
| `invoke` layering companion behavior                     | `define` and `design` seed or gap-record layering; `plan/full/validate` require layering artifact with layer decision snapshot       |
...
13. Implementation layering is treated as a default invoke companion artifact, with required coverage for `plan`, `full`, and `validate`.
14. Work-pack is treated as the stable invoke planning artifact, with required mapping from implementation-plan for `plan`, `full`, and `validate`.
15. Design mode is implemented as an L1 contract that emits six-view architecture artifacts, glossary consistency status, and plan-ready source design refs without creating work-pack tasks.
