# Invoke Example Prompt: architecture-medium

## Invocation

```text
arcanum-spell-invoke Design ecommerce checkout architecture with cart, payment, inventory, order, and failure compensation boundaries.
```
...
- Task ID: `architecture-medium`
- Template target: `architecture`
- Complexity: `medium`
- User request: Design ecommerce checkout architecture with cart, payment, inventory, order, and failure compensation boundaries.
- Expected invoke use: Emit six-view architecture plus interface and decision rules.
...
```text
arcanum/spells/invoke/development/example-outputs/architecture-medium.output.md
```
