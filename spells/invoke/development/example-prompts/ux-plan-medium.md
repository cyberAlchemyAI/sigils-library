# Invoke Example Prompt: ux-plan-medium

## Invocation

```text
arcanum-spell-invoke Plan ecommerce checkout UX across guest checkout, payment errors, address validation, and order confirmation.
```
...
- Task ID: `ux-plan-medium`
- Template target: `ux-plan`
- Complexity: `medium`
- User request: Plan ecommerce checkout UX across guest checkout, payment errors, address validation, and order confirmation.
- Expected invoke use: Emit UX plan with flows, state handling, and architecture handoff notes.
...
```text
arcanum/spells/invoke/development/example-outputs/ux-plan-medium.output.md
```
