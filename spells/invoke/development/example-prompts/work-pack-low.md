# Invoke Example Prompt: work-pack-low

## Invocation

```text
arcanum-spell-invoke Plan one small admin setting change with two tasks and one validation check.
```
...
- Task ID: `work-pack-low`
- Template target: `work-pack`
- Complexity: `low`
- User request: Plan one small admin setting change with two tasks and one validation check.
- Expected invoke use: Emit single-file work-pack.
...
```text
arcanum/spells/invoke/development/example-outputs/work-pack-low.output.md
```
