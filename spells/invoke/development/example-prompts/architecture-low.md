# Invoke Example Prompt: architecture-low

## Invocation

```text
arcanum-spell-invoke Design a read-only product catalog page service with search and category filters.
```
...
- Task ID: `architecture-low`
- Template target: `architecture`
- Complexity: `low`
- User request: Design a read-only product catalog page service with search and category filters.
- Expected invoke use: Emit six-view architecture plan.
...
```text
arcanum/spells/invoke/development/example-outputs/architecture-low.output.md
```
