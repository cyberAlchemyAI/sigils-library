# Invoke Example Prompt: sigil-medium

## Invocation

```text
arcanum-spell-invoke Create a fixture validation sigil that checks realistic requests, expected outputs, and run reports.
```
...
- Task ID: `sigil-medium`
- Template target: `sigil`
- Complexity: `medium`
- User request: Create a fixture validation sigil that checks realistic requests, expected outputs, and run reports.
- Expected invoke use: Emit sigil-development handoff with validation and observability requirements.
...
```text
arcanum/spells/invoke/development/example-outputs/sigil-medium.output.md
```
