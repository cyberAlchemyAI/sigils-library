# Invoke Example Prompt: spell-medium

## Invocation

```text
arcanum-spell-invoke Create an ecommerce release-readiness spell that composes implementation-layering, decision-gate, and task-session.
```
...
- Task ID: `spell-medium`
- Template target: `spell`
- Complexity: `medium`
- User request: Create an ecommerce release-readiness spell that composes implementation-layering, decision-gate, and task-session.
- Expected invoke use: Emit spell design handoff with phases, gates, and observability needs.
...
```text
arcanum/spells/invoke/development/example-outputs/spell-medium.output.md
```
