...
This workflow defines how a sigil moves from an idea to a maintained capability in the library.
...
Create `SKILL.md` from [sigil-template.md](sigil-template.md).
...
```bash
find sigils-library -path sigils-library/.git -prune -o -name "*.md" -print0 | xargs -0 -n1 ./tools/check_markdown_links.sh
rg -n -i "<project-specific-term>|<source-specific-term>|<legacy-label>" sigils-library --glob '!/.git/**'
```
